...
struct Home: View {
    ...
    @ViewBuilder
    func StackPlayrView(size: CGSize) -> some View {
        ...
        ZStack {
            ...
            ForEach(stackAlbums.reversed()) { album in
                ...
                // MARK: 3d Rotation
                    .rotation3DEffect(.degrees(expandCards && currentIndex != index ? -10 : 0), axis: (x: 1, y: 0, z: 0), anchor: .center, anchorZ: 1, perspective: 1)
                // Rotating Current card for 360 degree
                // You can customize this if you want rotation animation when closing
                    .rotation3DEffect(.degrees(showDetail && currentIndex == index && rotateCards ? 360 : 0), axis: (x: 1, y: 0, z: 0), anchor: .center, anchorZ: 1, perspective: 1)
                    .matchedGeometryEffect(id: album.id, in: animation)
                    .offset(y: CGFloat(index) * -20)
                    .offset(y: expandCards ? -CGFloat(index) * offsetHeight : 0)
                    .onTapGesture {
                        if expandCards {
                            // Selecting Curren Card
    ...
    // MARK: Detail View
    @ViewBuilder
    func DetailView(_ selectedAlbum: Album) -> some View {
        VStack(spacing: 0) {
            Button {
                rotateCards = false
                withAnimation {
            ...
            ScrollView(.vertical, showsIndicators: false) {
                VStack(spacing: 25) {
                    Image(selectedAlbum.image)
                        ...
                        .clipShape(RoundedRectangle(cornerRadius: 20, style: .continuous))
                    // Initial Rotation
                        .rotation3DEffect(.degrees(showDetail && rotateCards ? -180 : 0), axis: (x: 1, y: 0, z: 0), anchor: .center, anchorZ: 1, perspective: 1)
                        .rotation3DEffect(.degrees(animateDetailView && rotateCards ? 180 : 0), axis: (x: 1, y: 0, z: 0), anchor: .center, anchorZ: 1, perspective: 1)
                        .matchedGeometryEffect(id: selectedAlbum.id, in: animation)
                        .padding(.top, 50)
    ...
    // MARK: Album CardView
    @ViewBuilder
    func AlbumCardView(_ album: Album) -> some View {
        HStack(spacing: 12) {
            Image(album.image)
                .resizable()
                .aspectRatio(contentMode: .fill)
