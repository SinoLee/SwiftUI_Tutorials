                        ...
                        HStack(spacing: 10) {
                            ...
                            ForEach(taskModel.currentWeek, id: \.self) { day in
                                ...
                                VStack(spacing: 10) {
                                    ...
                                    Text(taskModel.extractDate(day, format: "dd"))
                                        .font(.system(size: 15))
                                        .fontWeight(.semibold)
                                    
                                    // EEE will return day as MON, TUE, ... etc
                                    Text(taskModel.extractDate(day, format: "EEE"))
                                        .font(.system(size: 14))
                                    
                                    Circle()
                                        .fill(.white)
                                        .frame(width: 8, height: 8)
                                        .opacity(taskModel.isToday(day) ? 1 : 0)
                                }
                                // MARK: Foreground Style
                                .foregroundStyle(taskModel.isToday(day) ? .primary : .secondary)
                                .foregroundColor(taskModel.isToday(day) ? .white : .black)
                                // MARK: Capsule Shape
